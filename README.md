# icarus-engine

### Original source of Godmode Indicator in Pinescript 3:
    - https://www.tradingview.com/script/oA3U7pok-GODMODE-OSCILLATOR-FRESH-BREAD-GENERATOR-FREE-TO-USE/
### Existing Python Godmode implementation reference (without willy/csi calculations for multiexchange ):
    - https://gist.github.com/iUmarov/4aa3558643ef156fe202f0b480f7050f

## EMA Component 
### Stackoverflow thread reference:
    - https://stackoverflow.com/questions/42869495/numpy-version-of-exponential-weighted-moving-average-equivalent-to-pandas-ewm/42905202 
    - https://stackoverflow.com/questions/48775841/pandas-ema-not-matching-the-stocks-ema
## TA without TA lib python reference:
    - https://www.quantopian.com/posts/technical-analysis-indicators-without-talib-code
## Python TA-lib reference:
    - https://mrjbq7.github.io/ta-lib/
